...
function CreateNewOrder() {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      if (!token) {
        console.log("No token, not fetching employees");
        return;
      }
      try {
  ...
  useEffect(() => {
    const fetchSpaces = async () => {
      ...
      const availableSpaces = allSpaces.filter(
        (space) => space.space_status === "available"
      );
      // Compare with previous available spaces
      if (
        prevSpacesRef.current.length &&
        availableSpaces.length > prevSpacesRef.current.length
      ) {
  ...
  const calculateOrderDescription = () => {
    return (selectedServices || [])
      .filter((service) => service && service.service_description)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!customerInfo || !customerInfo.customer_id) {
      console.error(
        "Customer info not loaded or missing customer_id",
        customerInfo
      );
      setErrorMessage("Customer info not loaded or missing customer_id");
      return;
    }
    console.log("vehicleInfo before submit:", vehicleInfo);
    const orderData = {
    ...
    console.log("customerInfo before submit:", customerInfo);
