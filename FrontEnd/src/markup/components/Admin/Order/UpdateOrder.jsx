...
function UpdateOrder() {
    ...
    if (!location.state?.order) {
      ...
      const fetchData = async () => {
        try {
          ...
          const formattedOrder = {
            ...fetchedOrder,
            order_id: fetchedOrder.order_id || orderId,
            order_description: fetchedOrder.order_description || "",
            order_services: Array.isArray(fetchedOrder.order_services)
              ? fetchedOrder.order_services
              : [],
            estimated_completion_date: formatDateForInput(
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setApiError(true);
      setApiErrorMessage("Please select at least one service.");
      return;
    }
    try {
      setSpinner(true);
