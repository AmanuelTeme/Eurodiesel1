...
import "./EmployeeWorkHoursReport.css";
import {
  Box,
  Typography,
  Autocomplete,
...
const EmployeeWorkHoursReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (!res.data.data || res.data.data.length === 0) {
        setError(
          t("No work hours found for this employee in the selected month.")
        );
      }
    } catch (err) {
  ...
  return (
    <Paper
      ...
      className="work-hours-report-root"
    >
      <Typography
        variant="h4"
        gutterBottom
        sx={{ fontWeight: 700, color: "#1a237e", textAlign: "center", mb: 3 }}
      >
        {t("Individual Employee Work Hours Report")}
      </Typography>
      <Box
        sx={{
          display: "flex",
      ...
      >
        <Autocomplete
          options={employees}
          getOptionLabel={(emp) =>
            `${emp.employee_first_name} ${emp.employee_last_name} (${emp.employee_email})`
          }
        ...
        <Button
          variant="outlined"
          onClick={downloadPDF}
          disabled={pdfLoading || !selectedEmployee || !report.length}
          sx={{
            minWidth: 140,
            fontWeight: 600,
            fontSize: "1rem",
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ background: "#f4f6fa" }}>
                <TableCell sx={{ fontWeight: 700 }}>{t("Truck")}</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>{t("Date")}</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>{t("Hours")}</TableCell>
                <TableCell sx={{ fontWeight: 700 }}>{t("Edit")}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {report.map((row, idx) => (
                <TableRow key={idx}>
                  <TableCell>
                    {row.vehicle_make} ({row.vehicle_serial})
                  </TableCell>
                  <TableCell>
                    {row.work_date && row.work_date.slice(0, 10)}
                  </TableCell>
      ...
      <Dialog open={editModalOpen} onClose={() => setEditModalOpen(false)}>
        ...
        <DialogContent>
          <Typography gutterBottom>
            <b>{t("Date")}:</b>{" "}
            {editRow?.work_date && editRow.work_date.slice(0, 10)}
            <br />
            <b>{t("Truck")}:</b> {editRow?.vehicle_make} (
            {editRow?.vehicle_serial})
          </Typography>
          <TextField
            label={t("Hours")}
            type="number"
