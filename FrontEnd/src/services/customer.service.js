...
async function createCustomer(formData, loggedInCustomerToken) {
  // define your headers
  const headers = {
    Authorization: `Bearer ${loggedInCustomerToken}`,
  };
...
const totalNofCustomers = async (token) => {
  ...
  const { data } = await axios.get(`${api_url}/api/total_customers`, {
    headers,
  });
...
const searchedCustomers = async (word, token) => {
  ...
  const { data } = await axios.get(`${api_url}/api/searched_customer/${word}`, {
    headers,
  });
