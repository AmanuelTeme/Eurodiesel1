...
async function checkIfEmployeeExists(email) {
  try {
    ...
    console.log("Rows returned for email check:", rows, "for email:", email);
...
async function createEmploye(employee) {
  ...
  employee.company_role_id = Number(employee.company_role_id);
  try {
    ...
    const [resultEmployee] = await connection.query(
      "INSERT INTO employee (employee_email,active_employee) VALUES (?,?)",
      [employee.employee_email, employee.active_employee]
    );
    console.log("Insert result for employee:", resultEmployee);
    if (resultEmployee.affectedRows !== 1) {
      return false;
    }
    const employee_id = resultEmployee.insertId;
    ...
    console.log("Insert result for employee_info:", resultInfo);

    // Insert into employee_pass
    const [resultPass] = await connection.query(
      "INSERT INTO employee_pass (employee_id, employee_password_hashed) VALUES (?,?)",
      [employee_id, hashedPassword]
    );
    console.log("Insert result for employee_pass:", resultPass);
    ...
    console.log("Insert result for employee_role:", resultRole);

    createdEmployee = { employee_id: employee_id };
  } catch (error) {
    console.log("Error in createEmploye:", error);
  }
