...
async function checkCustomerExists(customer_id) {
  ...
  console.log(`Query result for customer_id ${customer_id}:`, rows);
  ...
  try {
    ...
    if (space_id) {
      if (order_status == 1) {
        ...
        await conn.query(
          "UPDATE maintenance_space SET space_status = 'available' WHERE space_id = ?",
          [space_id]
        );
        // Unassign the space from the order
        await conn.query(
          "UPDATE orders SET space_id = NULL WHERE order_id = ?",
          [order_id]
        );
      } else {
        await conn.query(
          "UPDATE maintenance_space SET space_status = 'occupied' WHERE space_id = ?",
          [space_id]
        );
      }
    }
...
async function getAllOrders({ limit, sortby, completed }) {
  try {
    ...
    console.log(
      "Fetched orders (space_id, space_name, space_status):",
      orders.map((o) => ({
        order_id: o.order_id,
        space_id: o.space_id,
...
async function updateOrder(orderData, order_id) {
  try {
    ...
    const [currentOrder] = await conn.query(
      "SELECT space_id FROM orders WHERE order_id = ?",
      [order_id]
    );
    const oldSpaceId = currentOrder?.[0]?.space_id;

    // If assigning a new space, check if it's already occupied
    if (space_id && space_id !== oldSpaceId) {
      const [space] = await conn.query(
        "SELECT space_status FROM maintenance_space WHERE space_id = ?",
        [space_id]
      );
      if (space?.[0] && space[0].space_status === "occupied") {
    ...
    if (space_id) {
      if (order_status == 1) {
        // Use loose equality to handle string vs number
        console.log(
          "[updateOrder] Setting space",
          space_id,
          "to available and unassigning from order",
          order_id
        );
        await conn.query(
          "UPDATE maintenance_space SET space_status = 'available' WHERE space_id = ?",
    ...
    if (order_status == 1) {
      ...
      const [orderRow] = await conn.query(
        "SELECT assigned_employee_id, vehicle_id FROM orders WHERE order_id = ?",
        [order_id]
      );
      if (orderRow?.[0] && orderRow[0].assigned_employee_id) {
        ...
        await conn.query(
          ...
          [order_id, orderRow[0].assigned_employee_id]
        );
      }
    }
    ...
    if (order_status == 1) {
      // Use loose equality to handle string vs number
      console.log(
        "[Email Debug] Order status is 1, attempting to send completion email"
      );
      const customer = await getCustomerEmailByOrderId(order_id);
      console.log("[Email Debug] Customer data retrieved:", customer);
      if (customer && customer.customer_email) {
        ...
        const order_hash = hashResult?.[0] ? hashResult[0].order_hash : null;
        ...
        console.log("[Email Debug] Sending email to:", customer.customer_email);
        console.log("[Email Debug] AdminEmail config:", process.env.AdminEmail);
        console.log(
          "[Email Debug] PASS config:",
          process.env.PASS ? "Set" : "Not set"
        );

        if (!process.env.AdminEmail || !process.env.PASS) {
          console.error("[Email Debug] Email configuration missing!");
          return;
        }
        ...
        console.log(
          "[Email Debug] Customer email not found or customer data is null"
        );
      }
    }
...
async function getCustomerEmailByOrderId(order_id) {
  console.log(
    "[getCustomerEmailByOrderId] Looking for customer with order_id:",
    order_id
  );
  const query = `
  ...
  console.log("[getCustomerEmailByOrderId] Query result:", result);
  const customer = result?.[0] || null;
  console.log("[getCustomerEmailByOrderId] Returning customer:", customer);
  return customer;
}

// Update employee work hours by work_hour_id
async function updateEmployeeWorkHours(
  work_hour_id,
  ...
  const query = `UPDATE employee_work_hours SET ${fields.join(
    ", "
  )} WHERE work_hour_id = ?`;
