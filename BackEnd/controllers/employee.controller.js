...
async function updateEmployeeWorkHoursController(req, res) {
  try {
    ...
    const result = await updateEmployeeWorkHours(work_hour_id, {
      start_time,
      end_time,
      total_hours,
    });
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ error: "Work hours record not found or nothing to update." });
    }
    res
      .status(200)
      .json({ message: "Employee work hours updated successfully." });
  } catch (error) {
    res
      .status(400)
      .json({
        error: error.message || "Failed to update employee work hours.",
      });
  }
}
