...
async function updateOrder(req, res) {
  try {
    const { order_id } = req.params; // This is actually the order_hash
    const orderData = req.body;
    ...
    console.log("order_hash:", order_id);

    // Get the integer order_id from the hash
    ...
    const numericOrderId = orderResult[0].order_id;
    console.log("Numeric order_id:", numericOrderId);

    const result = await orderService.updateOrder(orderData, numericOrderId);
